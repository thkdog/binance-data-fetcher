...
export default function FuturesKlinesPage() {
  const [form] = Form.useForm();
  const storageKey = 'futures-kline-form-cache';
  ...
  useEffect(() => {
    ...
    try {
      ...
      console.error('Failed to restore futures kline form cache', err);
      form.setFieldsValue(defaults);
    }
  }, [form]);
  ...
  const handleValuesChange = (_, allValues) => {
    ...
    try {
      ...
      console.error('Failed to cache futures kline form', err);
    }
  };
  ...
  const onFinish = (values) => {
    ...
    window.location.href = `/api/futures/klines?${params.toString()}`;
  };
